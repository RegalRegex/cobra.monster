export const setAccordionHeight = (accordions: NodeListOf<HTMLDetailsElement>) => {
  const originalStates = Array.from(accordions).map((accordion) => accordion.open);
...
const assignHeight = (accordion: HTMLDetailsElement) => {
  ...
  accordion.style.setProperty("--ac-accordion-item-expanded", `${expandedHeight}px`);
  accordion.style.setProperty("--ac-accordion-item-collapsed", `${collapsedHeight}px`);
};

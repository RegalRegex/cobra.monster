...
export const blogHtml = ({ title, subtitle, date }: OgGenBlogProps): string => {
  ...
  <div class="w-[1200px] h-[630px] flex">
    ...
    <!-- Info Badge Top Left -->
    <div class="absolute flex -top-8 text-2xl "><h2 class="bg-[#7c0404] border-b-4 border-[#eab308] pl-1 pr-5 pt-3">${"Blog Post"}</h2></div>
      ...
      <div class="flex w-full justify-between m-10 h-full"> 
        ...
        <div class="flex flex-col w-[700px] h-[474px] justify-center pt-5">
          <h1 class="${title.length > 30 ? "text-7xl" : "text-8xl"} leading-[1.25] border-b-4 border-[#eab308] pb-10">${title}</h1>
        </div>
        <!-- Image -->
